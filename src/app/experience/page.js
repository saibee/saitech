  ...
  return (
    <div className="mx-auto max-w-6xl px-5 md:px-6 py-10">
      <div className="max-w-3xl">
        ...
        <h1 className="text-3xl md:text-4xl font-semibold tracking-tight mt-2">30 years of shipping, leading, learning.</h1>
        ...
        <div className="space-y-6">
          {experiences.map((e, i) => (
            <div key={i} className="relative flex gap-4 sm:gap-6">
              <div className="hidden sm:grid h-8 w-8 rounded-full bg-foreground text-background place-items-center text-[10px] font-bold mt-1 shrink-0 z-10">
              ...
              <div className="flex-1 rounded-[20px] border bg-background p-5 md:p-6 hover:shadow-md transition">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <h3 className="font-semibold mt-1">{e.company} <span className="font-normal text-muted-foreground">• {e.role}</span></h3>
                    <p className="text-xs text-muted-foreground mt-1">{e.location}</p>
                  </div>
                  <div className="flex flex-wrap gap-1.5 max-w-[260px] justify-end">
                    {e.tags.map((t) => <span key={t} className="text-[11px] px-2 py-1 rounded-full border bg-muted/50">{t}</span>)}
                  </div>
                </div>

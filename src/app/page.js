  ...
  return (
    <div>
          ...
          <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-10 items-center">
            <div>
              <div className="inline-flex items-center gap-2 text-xs px-3 py-1.5 rounded-full border bg-background">
                ...
                <span className="hidden sm:inline text-primary">• Pune / Remote</span>
              </div>
              <h1 className="mt-6 text-[34px] md:text-[52px] font-semibold tracking-tight leading-[0.95]">
                Frontend engineering
                <span className="block font-light text-primary">with 25 years of craft.</span>
              </h1>
              ...
              <div className="mt-6 flex flex-wrap gap-2 text-xs">
                {["React.js", "Next.js", "Sitecore", "Magnolia CMS", "Node.js", "AI-assisted Dev"].map((s) => (
                  <span key={s} className="px-2.5 py-1 rounded-full border bg-primary text-background">{s}</span>
                ))}
              </div>
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/projects" className="inline-flex items-center gap-2 px-5 py-3 rounded-full bg-primary text-background text-sm font-medium hover:opacity-90">
                  View projects <ArrowUpRight size={16} />
                </Link>
                <Link href="/contact" className="inline-flex items-center gap-2 px-5 py-3 rounded-full border bg-primary/89 text-background text-sm font-medium hover:bg-muted hover:text-primary">
                  <Mail size={16} /> Contact me
                </Link>
                <Link href="/about" className="inline-flex items-center gap-2 px-5 py-3 rounded-full border bg-primary/80 text-background text-sm font-medium hover:bg-muted hover:text-primary">
                  About <BadgeCheck size={16} />
                </Link>
              </div>
              <div className="mt-6 flex items-center gap-4 text-xs text-muted-foreground">
                <span className="inline-flex items-center gap-1.5"><Users size={14} className="text-primary" /> Managed 10 engineers</span>
                <span className="inline-flex items-center gap-1.5"><Layers size={14} className="text-primary" /> 5+ platforms</span>
                <span className="inline-flex items-center gap-1.5"><Sparkles size={14} className="text-primary" /> 3 concurrent projects</span>
              </div>
            </div>
            ...
            <div className="relative">
              <div className="rounded-[28px] border bg-background p-3 shadow-xl shadow-black/5">
                <div className="rounded-[20px] overflow-hidden border bg-muted aspect-[4/3] relative">
                  ...
                  <div className="absolute bottom-3 left-3 right-3 rounded-2xl bg-background/90 backdrop-blur p-3 border flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium leading-none text-primary">Saibal Biswas</p>
                      <p className="text-xs text-muted-foreground">Senior Team Lead</p>
                    </div>
                ...
                <div className="grid grid-cols-3 gap-3 mt-3">
                  {[
                    ...
                    <div key={s.k} className="rounded-2xl border bg-muted/40 p-3 text-center">
                      <p className="text-lg font-semibold leading-none text-primary">{s.k}</p>
              ...
              <div className="hidden md:flex absolute -right-2 -top-2 rounded-full border bg-background px-3 py-1.5 text-xs shadow-lg items-center gap-1.5">
                <span className="h-1.5 w-1.5 rounded-full bg-primary" /> EVERSANA • Feb 2024 – Present
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="border-y bg-muted/20">
        <div className="mx-auto max-w-6xl px-5 md:px-6 py-6 flex flex-wrap items-center justify-between gap-4 text-xs tracking-widest text-primary">
          <span>TRUSTED BY ENTERPRISE CLIENTS — US & EUROPE</span>
          <span className="flex gap-6 font-medium text-primary">
      ...
      <section className="mx-auto max-w-6xl px-5 md:px-6 py-12">
        <div className="flex items-end justify-between gap-4">
          <div>
            <h2 className="text-2xl font-semibold tracking-tight text-primary">Selected works</h2>
            <p className="text-sm text-muted-foreground mt-1">Thumbnails → click to open detailed case study with large imagery.</p>
          </div>
          <Link href="/projects" className="hidden sm:inline-flex items-center gap-1.5 text-sm font-medium hover:underline">All projects <ArrowUpRight size={14} className="text-primary" /></Link>
        </div>
          ...
          {projects.slice(0, 3).map((p) => (
            <Link key={p.slug} href={`/projects/${p.slug}`} className="group rounded-[20px] border bg-background overflow-hidden hover:shadow-lg transition">
              ...
              <div className="p-4">
                ...
                <p className="font-medium mt-1 leading-tight text-primary">{p.title}</p>
                <p className="text-sm text-muted-foreground mt-1 line-clamp-2">{p.excerpt}</p>
                <span className="inline-flex items-center gap-1 text-xs font-medium mt-3">View case <ArrowUpRight size={12} className="text-primary" /></span>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="mx-auto max-w-6xl px-5 md:px-6 pb-12">
        <div className="flex items-end justify-between gap-4">
          <h2 className="text-2xl font-semibold tracking-tight text-primary">Latest writing</h2>
          <Link href="/blog" className="text-sm font-medium hover:underline inline-flex items-center gap-1">All posts <ArrowUpRight size={14} className="text-primary" /></Link>
        </div>
        <div className="mt-6 grid md:grid-cols-3 gap-5">
          {blogPosts.slice(0, 3).map((post) => (
            <Link key={post.slug} href={`/blog/${post.slug}`} className="rounded-[20px] border bg-background overflow-hidden hover:shadow-md transition">
              ...
              <div className="p-4">
                ...
                <p className="font-medium leading-tight mt-1 text-primary">{post.title}</p>
      ...
      <section className="mx-auto max-w-6xl px-5 md:px-6 pb-16">
        <div className="rounded-[28px] border bg-foreground text-background p-6 md:p-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <div className="flex gap-3">
            ...
            <Link href="/projects" target="_blank" className="h-11 w-11 grid place-items-center rounded-full border border-white/20 bg-primary text-background hover:bg-primary/60"><Code2 size={16} /></Link>
          </div>
        </div>
      </section>
    </div>

...
export default function BlogPage() {
  return (
    <div className="mx-auto max-w-6xl px-5 md:px-6 py-10">
      ...
      <h1 className="text-3xl font-semibold tracking-tight mt-2">Writing — engineering, leadership, craft</h1>
          ...
          <Link key={post.slug} href={`/blog/${post.slug}`} className="group rounded-[22px] border bg-background overflow-hidden hover:shadow-lg transition">
            ...
            <div className="p-4">
              <div className="flex flex-wrap gap-1.5">
                {post.tags.map((t) => <span key={t} className="text-[11px] px-2 py-1 rounded-full border bg-muted/50">{t}</span>)}
              </div>
              <p className="text-xs text-muted-foreground mt-3">{post.date} • {post.read}</p>
              <h3 className="font-medium mt-1 leading-tight group-hover:underline">{post.title}</h3>

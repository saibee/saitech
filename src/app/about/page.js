  ...
  return (
    <div className="mx-auto max-w-6xl px-5 md:px-6 py-10">
      <div className="max-w-3xl">
        ...
        <h1 className="text-3xl md:text-4xl font-semibold tracking-tight mt-2 text-primary">Professional & personal — the long arc.</h1>
      ...
      <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-8 mt-10">
        <div className="rounded-[24px] border bg-background p-6 md:p-8">
          <h2 className="font-semibold flex items-center gap-2"><Briefcase size={16} className="text-primary" /> Professional snapshot</h2>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            <Link href="/experience" className="px-4 py-2 rounded-full bg-primary text-white text-sm font-medium">See full timeline →</Link>
            <Link href="/projects" className="px-4 py-2 rounded-full border bg-primary text-white text-sm font-medium hover:bg-muted hover:text-foreground">View projects</Link>
          </div>
        </div>

        <div className="rounded-[24px] border bg-muted/30 p-6 md:p-8">
          <h2 className="font-semibold flex items-center gap-2"><Heart size={16} className="text-primary" /> Personal</h2>
          ...
          <div className="mt-6 rounded-2xl border bg-background p-4 flex items-center gap-3">
            <img src={portrait_img.src || portrait_img} alt="" className="h-12 w-12 rounded-full object-cover" />
            <div>
            ...
            <span className="ml-auto text-xs px-2 py-1 rounded-full border bg-emerald-50 text-emerald-700 dark:bg-emerald-900/80 dark:text-emerald-300 text-center">Open to connect</span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8 grid md:grid-cols-4 gap-4">
        {skills.map((g) => (
          <div key={g.group} className="rounded-2xl border bg-background p-5">
            <p className="text-xs tracking-widest text-primary">{g.group.toUpperCase()}</p>
